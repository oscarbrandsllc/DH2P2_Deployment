  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    function buildLineupOptions(max, metric, teams) {
      ...
      const axisMax = metric === 'value'
        ? computeAxisMax(max, 5000, 0.06)
        : computeAxisMax(max, 5, 0.08);
        ...
        datasets,
        {
          ...
          scales: {
            x: {
              ...
              max: computeAxisMax(maxValue, 10000, 0.06),
            },
            y: {
              stacked: true,
              grid: { display: false },
    ...
    function computeAxisMax(value, step, paddingRatio = 0.05) {
      if (!Number.isFinite(value) || value <= 0) {
        return roundUpTo(0, step);
      }

      const padded = value * (1 + paddingRatio);
      return roundUpTo(padded, step);
    }
