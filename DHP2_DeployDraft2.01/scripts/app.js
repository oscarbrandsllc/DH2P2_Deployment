  ...
  `;
    ...
    hideLegend();
        ...
        function getConditionalColorByRank(rank, position) {
            ...
            const normalizedPos = typeof position === 'string' ? position.toUpperCase() : '';

            const rules = normalizedPos === 'WR'
                ? [
                    { threshold: 12, color: '#51CBA5DF' },
                    { threshold: 24, color: '#3EACD1' },
                    { threshold: 36, color: '#2382F7' },
                    { threshold: 48, color: '#957CFFB5' },
                    { threshold: 60, color: '#FF6FE1A5' },
                    { threshold: 72, color: '#FF2EB289' },
                    { threshold: Infinity, color: '#767693' },
                ]
                : [
                    { threshold: 8, color: '#51CBA5' },
                    { threshold: 16, color: '#3EACD1' },
                    { threshold: 24, color: '#2382F7' },
                    { threshold: 32, color: '#957CFF' },
                    { threshold: 44, color: '#FF6FE1' },
                    { threshold: 60, color: '#FF2EB2' },
                    { threshold: Infinity, color: '#767693' },
                ];

            for (const rule of rules) {
                if (rank <= rule.threshold) {
                    return rule.color;
                }
            }
