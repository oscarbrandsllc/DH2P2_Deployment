...
const SEASON_META_HEADERS = {
        ...
        function getSeasonRankValue(playerId, statKey) {
            ...
            const hasLiveOverrides = !!(state.liveWeeklyStats && Object.keys(state.liveWeeklyStats).length);

            if (statKey === 'fpts' || statKey === 'ppg') {
                if (hasLiveOverrides && typeof calculatePlayerStatsAndRanks === 'function') {
                ...
                if (statKey === 'fpts') {
                    ...
                    if (seasonStats) {
                        posRank = normalizeRank(seasonStats.pos_rank_ppr);
                    }

                    if ((posRank === null || posRank === undefined) && typeof calculatePlayerStatsAndRanks === 'function') {
                        const ranks = calculatePlayerStatsAndRanks(playerId);
                        if (ranks) {
                            posRank = normalizeRank(ranks.posRank);
                        }
                    }

                    return typeof posRank === 'number' && !Number.isNaN(posRank) ? posRank : null;
                }
                ...
                if (statKey === 'ppg') {
                    ...
                    let ppgRank = null;
                    if (seasonStats) {
                        ppgRank = normalizeRank(seasonStats.ppg_pos_rank_ppr || seasonStats.ppg_rank_ppr);
                    }

                    if ((ppgRank === null || ppgRank === undefined) && typeof calculatePlayerStatsAndRanks === 'function') {
                        const ranks = calculatePlayerStatsAndRanks(playerId);
                        if (ranks) {
                            ppgRank = normalizeRank(ranks.ppgPosRank);
                        }
                    }

                    return typeof ppgRank === 'number' && !Number.isNaN(ppgRank) ? ppgRank : null;
                }
            }
