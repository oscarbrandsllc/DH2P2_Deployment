    ...
    hideLegend();
        ...
        function parseSheetData(csvText) {
            ...
            rows.forEach(columns => {
                ...
                const heightDisplay = getColumnValue(columns, ['HEIGHT', 'HT', 'HT (FT-IN)', 'HEIGHT (FT-IN)', 'HEIGHT FT-IN']);
                const heightFeet = toFloat(getColumnValue(columns, ['HEIGHT_FT', 'HT_FT', 'HEIGHT (FT)', 'HT (FT)']));
                let heightInches = toFloat(getColumnValue(columns, ['HEIGHT_IN', 'HT_IN', 'HEIGHT (IN)', 'HT (IN)']));
                let heightTotalInches = toFloat(getColumnValue(columns, ['HEIGHT_TOTAL_IN', 'HT_TOTAL_IN', 'HEIGHT (TOTAL IN)', 'HT (TOTAL IN)', 'HEIGHT (INCHES)', 'HT (INCHES)']));
                if (Number.isFinite(heightInches) && heightInches >= 13 && !Number.isFinite(heightTotalInches)) {
                    heightTotalInches = heightInches;
                    heightInches = null;
                }
                const weightLbs = toFloat(getColumnValue(columns, ['WEIGHT', 'WT', 'WEIGHT (LBS)', 'WT (LBS)', 'WEIGHT_LBS', 'WT_LBS']));
                ...
                dataMap[sleeperId] = {
                    ...
                    height: heightDisplay || null,
                    heightFeet: Number.isFinite(heightFeet) ? heightFeet : null,
                    heightInches: Number.isFinite(heightInches) ? heightInches : null,
                    heightTotalInches: Number.isFinite(heightTotalInches) ? heightTotalInches : null,
                    weight: Number.isFinite(weightLbs) ? weightLbs : null
                };
            });
  ...
  `;
    ...
    hideLegend();
        ...
        function getPlayerVitals(playerId) {
            ...
            const formatHeightFromTotalInches = (totalInches) => {
            ...
            const parseHeight = () => {
                for (const entry of sheetEntries) {
                    const total = entry?.heightTotalInches;
                    if (Number.isFinite(total) && total > 0) {
                        const formatted = formatHeightFromTotalInches(total);
                ...
                for (const entry of sheetEntries) {
                    const feet = entry?.heightFeet;
                    const inches = entry?.heightInches;
                    if (Number.isFinite(feet) || Number.isFinite(inches)) {
                        pairCandidates.unshift([feet, inches]);
                    }
                }
                ...
                const heightCandidates = collect(
                    ...
                    ...sheetEntries.map(entry => entry?.heightTotalInches),
                    playerData.height,
            ...
            const rawPosition = playerData.position
