  ...
  `;
    ...
    hideLegend();
        ...
        function getPlayerVitals(playerId) {
            ...
            const collect = (...values) => values
                ...
                .filter(value => {
                    ...
                    if (typeof value === 'string') {
                        ...
                        if (upper === 'NA' || upper === 'N/A' || upper === 'NONE' || upper === '--' || upper === '-') {
                            return false;
                        }
                    }
                    return true;
            ...
            const formatHeightFromCentimeters = (centimeters) => {
            ...
            const parseHeight = () => {
                ...
                const centimeterCandidates = collect(
            ...
            const parseWeight = () => {
                ...
                const kilogramCandidates = collect(
                ...
                for (const candidate of kilogramCandidates) {
                    ...
                    if (Number.isFinite(numeric) && numeric > 0) {
                        ...
                        const display = `${rounded} lbs`;
                        return createValue(display, pounds);
                    }
                }
        ...
        function createPlayerVitalsElement(vitals, { variant = 'modal', position: positionOverride } = {}) {
            ...
            const ageColor = getAgeColorForVitals(resolvedPosition, safeVitals.age?.numeric ?? null);
            const heightColor = getHeightColorForVitals(resolvedPosition, safeVitals.height?.numeric ?? null);
            const weightColor = getWeightColorForVitals(resolvedPosition, safeVitals.weight?.numeric ?? null);
