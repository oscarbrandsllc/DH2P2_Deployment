  ...
  `;
    ...
    hideLegend();
        ...
        function getPlayerVitals(playerId) {
            ...
            const parseAge = () => {
                const sheetAges = sheetSnapshots
                    .map(snapshot => snapshot?.age)
                    .filter(age => age !== undefined && age !== null && age !== '');

                for (const candidate of sheetAges) {
                    const numeric = typeof candidate === 'number' ? candidate : Number.parseFloat(candidate);
