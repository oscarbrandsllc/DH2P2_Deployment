...
function showLegend(){ try{ document.getElementById('legend-section')?.classList.remove('hidden'); }catch(e){} }
    ...
    hideLegend();
        ...
        function scrollViewportToTop({ behavior = 'smooth' } = {}) {
            const performScroll = () => {
                const scrollOptions = { top: 0, behavior };
                let didScroll = false;
                ...
                extraTargets.forEach(target => {
                    ...
                    if (typeof target.scrollTo === 'function') {
                        try {
                            ...
                            // fall back to setting scrollTop below
                        }
                    }
                ...
                if (!didScroll) {
                    document.getElementById('header-container')?.scrollIntoView({ behavior, block: 'start', inline: 'nearest' });
                }
            };
