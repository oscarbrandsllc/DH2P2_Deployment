...
function showLegend(){ try{ document.getElementById('legend-section')?.classList.remove('hidden'); }catch(e){} }
    ...
    hideLegend();
        ...
        function scrollViewportToTop({ behavior = 'smooth' } = {}) {
            ...
            const performScroll = (scrollBehavior) => {
                if (typeof window.scrollTo === 'function') {
                    try {
                        ...
                        // Some browsers (older Safari) don't support object-based scrollTo with an options object.
                        window.scrollTo(0, 0);
                    }
                    return;
                }
            ...
            const fallbackBehavior = behavior === 'smooth' ? 'auto' : behavior;
            const schedule = typeof requestAnimationFrame === 'function'
                ? requestAnimationFrame
                : (cb) => setTimeout(cb, 0);
            schedule(() => performScroll(fallbackBehavior));
        }
...
const wrTeStatOrder = [
  ...
  'fum'
        ...
        function centerRosterGrid() {
            ...
            const container = rosterContainer || rosterGrid.parentElement;
            if (!container) {
                return;
            }

            // Clear previous adjustments before measuring widths.
            rosterGrid.style.paddingLeft = '';
            rosterGrid.style.paddingRight = '';
            ...
            if (gridWidth < availableWidth) {
                const totalGap = Math.max(availableWidth - gridWidth, 0);
                const startGap = Math.floor(totalGap / 2);
                const endGap = totalGap - startGap;
                rosterGrid.style.paddingLeft = `${startGap}px`;
                rosterGrid.style.paddingRight = `${endGap}px`;
            }
        }
