...
function showLegend(){ try{ document.getElementById('legend-section')?.classList.remove('hidden'); }catch(e){} }
        ...
        let headerStickyObserver = null;
        let headerOffsetUpdateFrame = null;

        const handleHeaderStickinessEntries = (entries) => {
        ...
        function createHeaderStickyObserver(offset) {
            if (typeof window === 'undefined' || !('IntersectionObserver' in window)) {
                headerStickyObserver = null;
                return;
            }

            if (headerStickyObserver) {
                headerStickyObserver.disconnect();
            }

            headerStickyObserver = new IntersectionObserver(handleHeaderStickinessEntries, {
                threshold: [0, 1],
                rootMargin: `${-(offset + 1)}px 0px 0px 0px`,
            });

            observedHeaderSentinels.forEach((sentinel) => headerStickyObserver.observe(sentinel));
        }

        function updateTeamHeaderStickyOffset() {
            if (pageType !== 'rosters' || !headerContainer) return;

            const offset = Math.max(0, Math.round(headerContainer.offsetHeight + 8));
            document.documentElement.style.setProperty('--team-header-offset', `${offset}px`);
            createHeaderStickyObserver(offset);
        }
        ...
        if (pageType === 'rosters') {
            ...
            if (typeof window !== 'undefined') {
                window.addEventListener('resize', scheduleTeamHeaderOffsetUpdate);
            }
        }
